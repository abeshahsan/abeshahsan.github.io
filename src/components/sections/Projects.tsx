...
export default function Projects() {
			...
			try {
				...
				const payloads = await Promise.all(
					PROJECT_SOURCES.map(async (source) => {
						...
						return entries.map((project) => ({ ...project, category: source.label, categoryId: source.id }));
					})
				);
				if (canceled) return;
	...
	return (
		<section id="projects" className="rounded-[2.25rem] border border-slate-200 bg-white/95 p-10 shadow-[0_40px_160px_-80px] shadow-slate-900/25 dark:border-slate-800 dark:bg-slate-950/80">
			<div className="space-y-8">
				<SectionHeader label="Projects" title="Selected work" copy="From ML pipelines to design systems, here are a few favorite builds." />
				<div className="flex flex-wrap gap-3">
					{["all", ...PROJECT_SOURCES.map((s) => s.id)].map((value) => {
						const label = value === "all" ? "All" : PROJECT_SOURCES.find((s) => s.id === value)?.label ?? value;
						return (
							<button
								key={value}
								type="button"
			...
			{activeProject ? <ProjectDialog project={activeProject} onClose={() => setActiveProject(null)} /> : null}
		</section>
	);
}
		...
		>
			...
			>
				<div className="flex items-start justify-between gap-4">
					...
					<button type="button" onClick={onClose} aria-label="Close project details" className="text-2xl">&times;</button>
				</div>
